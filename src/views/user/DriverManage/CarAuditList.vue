  ...
  <layout navActive="user">
    <div class="collection-settings-page">
      ...
      <el-table :data="tableData.data" border>
        ...
        <el-table-column label="行驶证" prop="drivingCertifyStateDesc">
          <template v-slot="scope">
            <span
              :class="{
        ...
        <el-table-column label="道路运输证" prop="roadLicenceCertifyStateDesc">
          <template v-slot="scope">
            <span
              :class="{
        ...
        <el-table-column label="操作" width="110">
          <template v-slot="scope">
            <el-button type="primary" @click="handleCarAduit(scope.row)">审核</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  async activated() {
    this.$store.dispatch('setBreadcrumb', [{ title: '首页', path: '/user' }, { title: '车辆资料审核' }]);
    this.handleSearch();
    this.setTimeMethod();
  },
  ...
  methods: {
    ...
    async fetchData(opts) {
      ...
      let res = await axios.post(api.vehicleCertifyList, options);
    ...
    handleDtl(row) {
      this.$router.push(`/certification/detail/${row.userCode}?pagetype=2`);
    },
    async handleCarAduit(row) {
      await axios.post(api.userAuditorRecord, { type: 2, userCode: row.userCode, vehicleId: row.vehicleId });
