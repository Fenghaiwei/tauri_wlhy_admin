  ...
  <layout navActive="user">
    <div class="collection-settings-page">
      ...
      <el-table :data="tableData.data" border>
        ...
        <el-table-column label="承运公司" prop="carrierCompany" min-width="220"> </el-table-column>
        ...
        <el-table-column label="身份证" prop="idCardAuditState" min-width="150">
          <template v-slot="scope">
            <span v-if="scope.row.idCardAuditState == 1" class="font-defalut"> 通过 </span>
            <span v-if="scope.row.idCardAuditState == 2" class="font-red">
              不通过 <span v-if="scope.row.idCardRejectReason">({{ scope.row.idCardRejectReason }})</span>
            </span>
          </template>
        </el-table-column>
        <el-table-column label="驾驶证" prop="driverLicenceAuditState" min-width="150">
          <template v-slot="scope">
            <span v-if="scope.row.driverLicenceAuditState == 1" class="font-defalut"> 通过 </span>
            <span v-if="scope.row.driverLicenceAuditState == 2" class="font-red">
              不通过 <span v-if="scope.row.driverLicenceRejectReason">({{ scope.row.driverLicenceRejectReason }})</span>
            </span>
          </template>
        </el-table-column>
        <el-table-column label="从业资格证" prop="qualificationAuditState" min-width="150">
          <template v-slot="scope">
            <span v-if="scope.row.qualificationAuditState == 1" class="font-defalut"> 通过 </span>
            <span v-if="scope.row.qualificationAuditState == 2" class="font-red">
              不通过 <span v-if="scope.row.qualificationRejectReason">({{ scope.row.qualificationRejectReason }})</span>
            </span>
          </template>
        </el-table-column>
...
export default {
  ...
  async activated() {
    this.$store.dispatch('setBreadcrumb', [{ title: '首页', path: '/user' }, { title: '司机复核记录' }]);
  },
  methods: {
    handleSearchFn() {
      if (
    ...
    async fetchData(opts) {
      ...
      let res = await axios.post(api.pageDriverCarrierReviewed, options);
